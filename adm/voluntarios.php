...
$sql = "SELECT v.id, v.nome, v.email, v.telefone, v.habilidades, COALESCE(p.presencas, 0) AS presencas 
        FROM voluntarios v 
        LEFT JOIN (
        SELECT id_voluntario, COUNT(*) as presencas 
        FROM presencas 
        GROUP BY id_voluntario) p 
        ON v.id = p.id_voluntario";
