...
<main>
        ...
        <div class="flex items-center justify-between px-5 py-3 border-b border-borda">
            <div class="flex flex-col justify-start items-left gap-2 w-full">
                ...
                <div class="w-full flex flex-col gap-2">
                    <div class="flex justify-between items-center">
                        ...
                        <p class="text-base text-texto-preto">R$ <?= htmlspecialchars(metas($row["id"])) ?> /
                            R$ <?= htmlspecialchars(intval($row["meta_valor"])) ?></p>
                    </div>
                    <?php
            ...
            <div class="flex flex-col w-15 justify-center items-center">
                ...
                <form action="deletar_recursos" method="POST">
                    ...
                    <button class="w-10 h-10 rounded-lg hover:bg-red-100 text-red-500 cursor-pointer" type="submit">
    ...
    <div class="mt-5 border border-borda rounded-lg shadow-lg bg-white">
        <p
            class="border-b border-borda px-5 py-5 flex justify-between items-center font-semibold text-texto-preto text-lg">
            Últimas Doações Recebidas</p>
        ...
        <div class="flex items-center justify-between px-5 py-3 border-b border-borda">
            <div class="flex flex-col justify-start items-left gap-1">
                <h3 class="text-lg font-bold text-texto-preto flex justify-center items-center gap-2">
                    <?= htmlspecialchars($row["nome_doador"]) ?><span
                        class="text-base font-normal text-texto-opaco"><?= htmlspecialchars($row["data_doacao"]) ?></span>
                </h3>
                <p class="text-base text-texto-opaco"><?= htmlspecialchars($row["numero_contato"]) ?></p>
            </div>
            <p class="font-bold text-green-600 text-lg">R$ <?= htmlspecialchars($row["valor"]) ?></p>
        </div>
        <?php endwhile; ?>
        <?php else: ?>
        <div class="flex items-center justify-between px-5 py-3">
